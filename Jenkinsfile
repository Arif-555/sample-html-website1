pipeline {
    ...
    environment {
        ...
        DEV_INSTANCE = '13.229.135.49'      // EC2 IP for Dev Environment
        STAGING_INSTANCE = '13.215.248.29'  // EC2 IP for Staging Environment
    ...
    parameters {
        ...
        choice(name: 'ENVIRONMENT', choices: ['dev', 'stagging', 'main', 'prod'], description: 'Select the Environment to deploy')
    }
        ...
        stage('Setup Environment') {
            steps {
                script {
                    ...
                    def repoBranch = ""
                    def repoDirectory = "/var/www/html/sample-html-website"
                    def INSTANCE_IP = ""
                    ...
                    sshagent(credentials: ['github-ssh-key']) {
                        sh """
                            ...
                            if [ ! -d "${repoDirectory}" ]; then
                                ...
                                git clone --branch ${repoBranch} git@github.com:Arif-555/sample-html-website1.git ${repoDirectory}
                            else
